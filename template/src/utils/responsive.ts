...
const DEFAULT_REFERENCE_WIDTH = 375;
const DEFAULT_REFERENCE_HEIGHT = 812;
...
export const configureScaler = (config: { referenceWidth?: number; referenceHeight?: number }): void => {
...
export const responsive = {
  width: (value: number, options?: { min?: number; max?: number }) => scaleWidth(value, options),

  height: (value: number, options?: { min?: number; max?: number }) => scaleHeight(value, options),
  ...
  radius: (value: number, options?: { min?: number; max?: number }) => scaleRadius(value, options),
...
export const onOrientationChange = (callback: () => void): { remove: () => void } => {
